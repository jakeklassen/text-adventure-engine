...
function processExpression(expression = {}) {
  try {
    ...
    if (expression.player_pickup_item) {
      const objectId = expression.player_pickup_item;

      const room = Game.rooms.find(({ id }) => id === Game.player.current_room);
    ...
    switch (command) {
      ...
      case 'inventory': {
        ...
        if (items.length === 0) {
          ...
          items.map(item => console.log(' - ', item.inventory_description));
        }
